...
namespace objsamples
{
    partial class Tester
    {
        static void TestET_Campaign()
        {
            ...
            var exampleAssetType = "CMS_ASSET";

            Console.WriteLine("--- Testing Campaign ---");

            Console.WriteLine("\n Retrieve All Campaigns");
            var getCampaign = new ETCampaign
            {
                AuthStub = myclient,
            };
            ...
            foreach (ETCampaign thisCamp in grCampaign.Results)
            ...
            {
                var campaign = (ETCampaign)prCampaign.Results[0].Object;
                ...
                var singleCampaign = new ETCampaign
                {
                    AuthStub = myclient,
                    ID = campaign.ID,
                };
                ...
                Console.WriteLine("\n Create a new Asset");
                var asset = new ETAsset()
                {
                    AuthStub = myclient,
                    Name = "Asset-Sample" + Guid.NewGuid().ToString(),
                    AssetType = new AssetType { Id = 207, Name = "templatebasedemail" }
                };
                var prAsset = asset.Post();
                Console.WriteLine("Post Status: " + prAsset.Status.ToString());
                Console.WriteLine("Message: " + prAsset.Message);
                Console.WriteLine("Code: " + prAsset.Code.ToString());
                Console.WriteLine("Results Length: " + prAsset.Results.Length);
                var createdAsset = (ETAsset)prAsset.Results[0].Object;

                Console.WriteLine("\n Create a new Campaign Asset");
                var postCampAsset = new ETCampaignAsset
                {
                    AuthStub = myclient,
                    CampaignID = idOfpostCampaign,
                    Type = exampleAssetType,
                    IDs = new[] { createdAsset.ID.ToString() },
                };
                ...
                if (prCampAsset.Status)
                {
                    ...
                    var singleCampAsset = new ETCampaignAsset
                    {
                        AuthStub = myclient,
                    ...
                    var deleteCampAsset = new ETCampaignAsset
                    {
                        AuthStub = myclient,
                        ID = Convert.ToInt16(idOfpostCampaignAsset),
                        CampaignID = idOfpostCampaign,
                ...
                Console.WriteLine("\n Delete Campaign");
                var delCampaign = new ETCampaign
                {
                    AuthStub = myclient,
                    ID = campaign.ID,
                };
